...
const MENU = [
  { title: '홈',    items: [{ label: '대시보드', view: VIEWS.DASHBOARD, icon: Home }] },
  { title: '관리비', items: [
...
export default function DemoTenant({ onLoginRequired, autoStartTutorial = false }) {
  ...
  return (
    <TutorialProvider viewKey={view} autoStart={autoStartTutorial}>
      <DemoTenantContent onLoginRequired={onLoginRequired} view={view} setView={setView} />
    </TutorialProvider>
  );
}

// TutorialProvider 안쪽 실제 컨텐츠
function DemoTenantContent({ onLoginRequired, view, setView }) {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm transition-colors ${
                    active ? 'bg-blue-600 text-white font-semibold' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700'
                  }`}
                >
